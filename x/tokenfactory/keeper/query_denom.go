...
func (s queryServer) ListDenom(ctx context.Context, req *types.QueryAllDenomRequest) (*types.QueryAllDenomResponse, error) {
	if req == nil {
	...
	pageRes, err := query.Paginate(denomStore, req.Pagination, func(key []byte, value []byte) error {
		...
		if err := s.k.cdc.Unmarshal(value, &denom); err != nil {
			return err
		}
...
func (s queryServer) GetDenom(ctx context.Context, req *types.QueryGetDenomRequest) (*types.QueryGetDenomResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "invalid request")
	}

	val, found := s.k.GetDenom(
		ctx,
		req.Denom,
	)
	if !found {
